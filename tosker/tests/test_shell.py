...
class Test_Shell(unittest.TestCase):
    ...
    def test_args_flags(self):
        ...
        error, file, cmds, comps, flags, inputs = shell._parse_unix_input(args)
    ...
    def test_args_command(self):
        args = ['test.yaml', 'create', 'start', 'stop', 'delete']
        error, file, cmds, comps, flags, inputs = shell._parse_unix_input(args)
        self.assertIsNone(error)
        self.assertEqual('test.yaml', file)
        ...
        args = ['test.yaml', 'create', 'test', 'start', 'stop', 'delete']
        error, file, cmds, comps, flags, inputs = shell._parse_unix_input(args)
        self.assertIsNotNone(error)
